...
suite =
    textBasedTestSource
        |> String.split "\n\n"
        |> List.indexedMap parseDay
        |> List.foldl
            (\( dayNumber, day ) ( maybePreviousDay, tests ) ->
                case maybePreviousDay of
                    Nothing ->
                        ( Just day, tests )
                        ...
                        let
                            ...
                            test =
                                describe
                                    ...
                                    (List.map2 toTest previousDay day)
                        in
                        ( Just day
                        , test :: tests
                        )
        ...
        |> Tuple.second
        |> List.reverse
        |> describe "Check updateItemQuality"


toTest : Item -> Item -> Test
toTest previous current =
    test
        (itemToString previous ++ " should become " ++ itemToString current)
        (\() ->
            updateItemQuality previous
...
itemToString item =
    item.name ++ ", " ++ String.fromInt item.sellIn ++ ", " ++ String.fromInt item.quality


parseDay : Int -> String -> ( Int, List Item )
parseDay index day =
    ( index
    , day
        |> String.split "\n"
...
parseItem line =
    case String.split ", " line of
        [ name, sell_by_string, quality_string ] ->
            case ( String.toInt sell_by_string, String.toInt quality_string ) of
                ( Just sell_by, Just quality ) ->
                    Just { name = name, sellIn = sell_by, quality = quality }
